...
namespace RaspberryGPIOManager
{
    ...
    public class GPIOPinDriver : IDisposable
    {
            ...
            private set
            {
                lock (_exported_pins)
                {
                    if (_disposed)
                        ...
                        throw new ObjectDisposedException("Requested pin is already exported.");
                    else
                    {                                                                                       //  0  1  2  3  4  5
        ...
        /// <summary>
        /// Function in <see cref="GPIOPin"/> to Dispose the <see cref="RaspberryGPIOManager.GPIOPinDriver"/>.
        /// </summary>
        public void Dispose()
        {
            if (_disposed)
        ...
        {
            //700MHz
            int frequency = ConfigReader.getConfig().pwm_frequency;
            var speaker = new RaspberryGPIOManager.GPIOPinDriver(pin);
            speaker.Direction = RaspberryGPIOManager.GPIOPinDriver.GPIODirection.Out;

            float halfPeriod = 500000 / frequency;
            float looptime = 1 / halfPeriod * 2;
            int counting = (int)(200000 * duration * looptime / 16f);
            ...
            for (int dur = 0; dur < duration; dur++)
            {
                for (int i = 0; i < counting; i++)
                {
                    ...
                    else
                    {
                        ...
                        speaker.State = RaspberryGPIOManager.GPIOPinDriver.GPIOState.High;
                        Thread.Sleep((int)(halfPeriod / 1000));
                        speaker.State = RaspberryGPIOManager.GPIOPinDriver.GPIOState.Low;
                        Thread.Sleep((int)(halfPeriod / 1000));
                    }
                } 
            }
